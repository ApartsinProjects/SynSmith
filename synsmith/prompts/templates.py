...
UPDATER_SYSTEM = (
    ...
    "You receive the current prompt and structured feedback from several critics "
    "(attribute verifier, realism discriminator, diversity auditor, pack discriminator, "
    "mode hunter, coverage hole finder). "
    ...
    "CRITIC-SIGNAL PRIORITY (Fix F4 / F5 / register stability):\n"
    "1. Coverage-hole exemplars are the HIGHEST-PRIORITY positive signal. They are "
    "actual surface patterns from the target distribution that the synth is missing. "
    "The rewritten prompt MUST include a 'Preferred phrasings' block listing the "
    "coverage-hole exemplars verbatim (or close paraphrases). Failure to promote "
    "coverage-hole exemplars is the single most common updater bug.\n"
    "2. Pack-level patterns are LOWER-PRIORITY and must be FILTERED before use. Each "
    "pack pattern claims that the synthetic batch repeats some surface property; some "
    "such properties (e.g. 'short, direct sentences', 'informal tone with minimal "
    "punctuation') CHARACTERISE THE TARGET DISTRIBUTION, not a synthesis artifact. "
    "Before adding any pack-level pattern to a 'Forbidden phrasings' block, check whether "
    "the same property is visible in the coverage-hole exemplars above. If yes, the "
    "pack pattern is a target-register signal and must be IGNORED (treating it as a "
    "ban would push the generator away from the target distribution).\n"
    "3. Mode-hunter banned phrasings are deterministic and SAFE to add to the ban list "
    "without further filtering (they have already passed a real-vocab veto).\n"
    "4. Realism artifacts are SAFE positive guidance only when consistent across "
    "iterations; a single iter's discriminator reasons may reflect a small real-anchor "
    "subset and should not be over-weighted."
)
